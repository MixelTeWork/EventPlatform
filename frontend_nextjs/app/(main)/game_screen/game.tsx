...
class Game
{
	...
	private counterV = 0;
	private startI = 0;
	private counterTimer?: NodeJS.Timeout;
	private stoped = false;

	public start(updateScreen: () => void, router: AppRouterInstance)
	{
		this.stoped = false;
		this.startI++;
		this.state = null;
	...
	private onNewState()
	{
		...
		if (!this.state.showGame || this.state.state == "tourneyEnd")
			this.router?.replace("/tourney_screen");
	}
...
export function useGame()
{
	...
	useEffect(() =>
	{
		game.start(() => update.set(v => v + 1), router);
		return () => game.stop();
